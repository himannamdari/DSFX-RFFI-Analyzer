...
def train_and_evaluate_ridge(data_path, top_features):
    # Load the dataset
    ...
    # Initialize and train a Ridge Regression model using raw data
    raw_ridge = Ridge(alpha=1.0)
    start_time = time.time()
    raw_ridge.fit(X_train_scaled, y_train)
    raw_training_time = time.time() - start_time

    # Initialize and train a Ridge Regression model using extracted features
    ext_ridge = Ridge(alpha=1.0)
    start_time = time.time()
    ext_ridge.fit(X_train_scaled, y_train)
    ext_training_time = time.time() - start_time

    # Initialize and train a Ridge Regression model using PCA data
    pca_ridge = Ridge(alpha=1.0)
    start_time = time.time()
    pca_ridge.fit(pca_data, y_train)
    pca_training_time = time.time() - start_time

    # Predictions
    raw_train_preds = raw_ridge.predict(X_train_scaled)
    raw_test_preds = raw_ridge.predict(X_test_scaled)

    ext_train_preds = ext_ridge.predict(X_train_scaled)
    ext_test_preds = ext_ridge.predict(X_test_scaled)

    pca_test_preds = pca_ridge.predict(pca_test_data)
...
results = train_and_evaluate_ridge(data_path, top_features)
